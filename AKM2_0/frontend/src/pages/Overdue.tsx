...
import { ChevronsLeft, ChevronLeft, ChevronRight ,ChevronsRight, X, Menu, Globe, Calendar, ChevronDown, RefreshCw, ArrowUp, ArrowDown, Columns3, Download } from 'lucide-react';
...
const allColumns = [
  ...
  { key: 'overdue_date', label: 'Overdue Date', width: 'min-w-36' },
...
const OverduePage: React.FC = () => {
  ...
  // Auto-navigate to list view when selectedDate or overdueDate changes on mobile
  useEffect(() => {
    if (isMobile && (selectedDate !== 'All' || overdueDateFrom || overdueDateTo)) {
      setMobileViewMode('list');
    }
  }, [selectedDate, overdueDateFrom, overdueDateTo, isMobile]);
  const isFullyLoaded = totalCount === 0 || overdueRecords.length >= totalCount;
  ...
  const [sortColumn, setSortColumn] = useState<string | null>('overdue_date');
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    const saved = localStorage.getItem('overdueColumnOrder');
    if (saved) {
      try {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(() => {
    const saved = localStorage.getItem('overdueVisibleColumns');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) return parsed;
  ...
  const globalFilteredRecords = React.useMemo(() => {
    ...
    if (userOrgId) {
      filtered = filtered.filter((record: Overdue) => record.organization_id === userOrgId);
    } else {
      filtered = filtered.filter((record: Overdue) => !record.organization_id);
    }
    ...
    return filtered;
  }, [overdueRecords, searchQuery, overdueDateFrom, overdueDateTo, userOrgId]);
  ...
  useEffect(() => {
    if (allColumns.length > 0 && columnOrder.length === 0) {
      ...
      setVisibleColumns(['id', 'account_no', 'full_name', 'overdue_date', 'invoice_id']);
    }
  }, [columnOrder.length]);
  ...
  const handleToggleColumn = (columnKey: string) => {
    setVisibleColumns(prev => {
      const next = prev.includes(columnKey)
        ...
        : [...prev, columnKey];
      localStorage.setItem('overdueVisibleColumns', JSON.stringify(next));
      return next;
    });
  };
  ...
  const handleSelectAllColumns = () => {
    ...
    localStorage.setItem('overdueVisibleColumns', JSON.stringify(allKeys));
  };

  const handleDeselectAllColumns = () => {
    setVisibleColumns([]);
    localStorage.setItem('overdueVisibleColumns', JSON.stringify([]));
  };
  ...
  const handleDrop = (e: React.DragEvent, targetColumnKey: string) => {
    ...
    localStorage.setItem('overdueColumnOrder', JSON.stringify(newOrder));
    setDraggedColumn(null);
    setDragOverColumn(null);
  };
  ...
  const filteredRecords = React.useMemo(() => {
    ...
    if (sortColumn) {
      filtered = [...filtered].sort((a, b) => {
        ...
        } else if (sortColumn === 'overdue_date') {
  ...
  const renderCellValue = (record: Overdue, columnKey: string) => {
    switch (columnKey) {
      case 'id':
        return record.id;
      case 'account_no':
      ...
      case 'overdue_date':
        return formatDate(record.overdue_date);
      case 'invoice_id':
  ...
  const handleExport = () => {
    ...
    const getExportValue = (record: Overdue, columnKey: string) => {
      return renderCellValue(record, columnKey);
    };

    exportToCSV('overdue_export', exportColumns, filteredRecords, getExportValue);
  };
          ...
          <div className={`p-4 border-b flex-shrink-0 ${isDarkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'
            ...
            <div className="flex items-center justify-between space-x-3 overflow-x-auto scrollbar-none pb-1 -mb-1 w-full">
              ...
              <button
                ...
                title={!isFullyLoaded ? `Loading records... (${overdueRecords.length}/${totalCount})` : isRefreshingManual ? "Checking for updates..." : "Refresh Records"}
                className="relative p-2 rounded-lg transition-all duration-200 flex items-center justify-center shadow-sm disabled:opacity-50 border flex-shrink-0"
                style={{
                  backgroundColor: '#ffffff',
                  borderColor: colorPalette?.primary || '#7c3aed',
